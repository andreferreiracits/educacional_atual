...
<asp:Content ContentPlaceHolderID="PageJsArea" ID="PageJsArea" runat="server">   
    ...
    <script src="/AVA/StaticContent/Common/Scripts/timeline_3.1.17.js<%=Url.TimeStampLink() %>" type="text/javascript"></script>
    
    <script type="text/javascript" src="/AVA/StaticContent/Common/Scripts/jquery.AvaSelector_3.4.5.js<%=Url.TimeStampLink() %>"></script>
...
<asp:Content ContentPlaceHolderID="ContentArea" ID="ContentArea" runat="server">
   ...
   <span class="mi_actions">
       ...
       <%  
           if (Model.idVisitante != Model.idUsuario || (bool)ViewData["admRede"])
           {
               if ((bool)ViewData["bolExcluido"] == false && (int)ViewData["idEstado"] != 6)
               { 
               %>
        ...
        %>
           ...
           <h2 class="blokletters"><%=Model.strNome %></h2>
           <%
               if (Model.bolAluno)
               {
                   %>
                   <h4 class="blokletters"><%=Model.strTurma %></h4>
                   <%
               }
            %>
...
<!-- AQUI INICIA AREA DO MURAL -->
    <section class="hs1">		        
        <section class="timeline">
            <header>
                <h1 class="tituloMural roboto"><div class="icon_li mural"></div>MINHAS PUBLICAÇÕES</h1>
                <% 
