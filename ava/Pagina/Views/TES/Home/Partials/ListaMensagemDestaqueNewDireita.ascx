...
<div class="bloco_conteudo destaque">
    ...
    <nav class="nav_carrosel">
			<a href="" class="prev" id="next_destD_simples"></a>
			<a href="" class="next" id="prev_destD_simples"></a>
		</nav>
    <%
        }
    %>
    <div id="lista_destaqueD_simples" class="carrosel">
    ...
    <%
        if (Model.Count > 0)
        {
            var order = 1;
    %>
    <nav id="nav_footer_destaqueD_simples" max="<%=Model.Count %>" class="nav_footer">
			<a href="/AVA/Pagina/<%=objPagina.strLink %>#Destacados" class="opcao_vertodos">Veja mais</a>
			<ul>
                <%foreach (var f in Model)
                  {%>
				    <li id="<%=order %>" order="<%=order %>" <%if(order == 1){ %>class="ativo"<%} %>><a id="<%=order %>"></a></li>
                <%
                      order += 1;
                  }
                %>
