...
function addFiltroAgendador() {
    ...
    if ($("#idAgendador option:selected").val() != 0) {
        $("<input>").attr({ "type": "hidden", "name": "txtidAgendador" }).val($("#idAgendador option:selected").val()).appendTo(
            ...
            .html($("#idAgendador option:selected").text() + " <img onclick='excluirFiltroAgen(this);' src= '" + pathImgFechar() + "' />")
...
function addFiltroPeriodo() {
    ...
    if ((vRI.val() != "") && (vRF.val() != "") && (vHI.val() != "") && (vHF.val() != "")) {
        var cSP = $("<span>").attr({ "class": "spanFiltro" }).html("Período: " + $("#realizacaoInicial").val() + " " + $("#horaInicial").val() + " até " + $("#realizacaoFinal").val() + " " + $("#horaFinal").val() + " <img onclick='excluirFiltroPeri(this);' src='" + pathImgFechar() + "' />");
        vRI.appendTo(cSP);
        vRF.appendTo(cSP);
        vHI.appendTo(cSP);
        vHF.appendTo(cSP);
...
function pathImgFechar() {
    var imgCalendario = $("#pathImgFechar");
    var pathImagem = caminhoBase + "/" + "Content/imgcss/1.0.2/bt_x.png";
    if (imgCalendario.length > 0) {
        pathImagem = imgCalendario.attr('src');
    }
    return pathImagem;
}