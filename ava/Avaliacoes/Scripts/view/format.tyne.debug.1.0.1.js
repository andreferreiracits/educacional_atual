...
//        auto_reset_designmode : true,
        ...
        theme_advanced_buttons2: "outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,|,tablecontrols,|,hr,|,sub,sup,|,charmap,|,image,|,banco_imagens,|,media",//,simuladoravaliacoesnova, media
...
{
    plugins: "banco_imagens,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template,advlist,htmlcharcount",
    file_browser_callback: "carregarPopupImagemQuestao",
    ...
    formatIntroducaoProva = $.extend({}, formatDefault,
{
    ...
    file_browser_callback: "carregarPopupImagemProva"
});
    formatInstrucaoAplicacao = $.extend({}, formatDefault, {
        plugins: "banco_imagens,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template,advlist,htmlcharcount",
        file_browser_callback: "carregarPopupImagemAplicacao"
    });
...
formatRealizacao = {
    ...
    file_browser_callback: "carregarPopupImagemRealizacao",
...
/* somente para o modo debug e para o net q está com pau no upload asp antigo */
function carregarCaminhoImagem(caminho) {
    popupImagem.win.document.forms[0].elements[popupImagem.field].value = caminho;
}
function carregarPopupImagemAplicacao(field, url, type, win) {
    var wMaxImageResize = 500;
    var hMaxImageResize = 375;
    var wenv;
    var data = new Date().getDate();
    var id = $("#idAplicacaoSalvar").val();

    

    popupImagem = { 'field': field, 'win': win };

    wenv = window.open(caminhoBase + "/BaseAplicacao/AnexarImagem/" + id, "wndEnviar_" + data, "height=190,width=380");
    wenv.focus();
}
function carregarPopupImagemQuestao(field, url, type, win) {
    var wMaxImageResize = 500;
    var hMaxImageResize = 375;
    var wenv;
    var data = new Date().getDate();
    var id = $("#idQuestaoSalvar").val();

    popupImagem = { 'field': field, 'win': win };

    wenv = window.open(caminhoBase + "/Questoes/AnexarImagem/" + id, "wndEnviar_" + data, "height=190,width=380");
    wenv.focus();
}


function carregarPopupImagemProva(field, url, type, win) {
    var wMaxImageResize = 500;
    var hMaxImageResize = 375;
    var wenv;
    var data = new Date().getDate();
    var id = $("#idProvaSalvar").val();

    popupImagem = { 'field': field, 'win': win };

    wenv = window.open(caminhoBase + "/Criacao/AnexarImagem/" + id, "wndEnviar_" + data, "height=190,width=380");
    wenv.focus();
}

function carregarPopupImagemRealizacao(field, url, type, win) {
    
    var wMaxImageResize = 500;
    var hMaxImageResize = 375;
    var wenv;
    var data = new Date().getDate();
    var id = $("#idProvaRealizacao").val();

    popupImagem = { 'field': field, 'win': win };

    wenv = window.open(caminhoBase + "/Realizacao/AnexarImagem/" + id, "wndEnviar_" + data, "height=190,width=380");
    wenv.focus();
}
