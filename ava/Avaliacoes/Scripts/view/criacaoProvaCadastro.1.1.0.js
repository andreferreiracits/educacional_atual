...
function inicializar() {
    ...
    $('#btnConcluirImpressaoHtml').click(imprimirAvaliacaoHtml)
...
function retornarAcaoImprimirHtml(dados) {
    carregando.esconder();
    if (retornoErro(dados)) {
        return;
    }
    
    if ($(dados).attr('id') == 'modelos') {
        $("#dlgImpressao #boxDownloadModelos").html($(dados).html());
        $("#dlgImpressao #boxDownloadModelos").show();
        ...
        var w = window.open('', 'imprimir', 'width=800, height=600');
        w.document.open();
        w.document.write(dados);
        w.document.close();
        //$(w.document.html).html(dados);
    }
...
function imprimirAvaliacaoHtml(evt) {
    if (evt) { evt.preventDefault(); }
    carregando.mostrar();
    $.ajax({
        url: caminhoBase + '/Criacao/ImprimirHtml/',
        data: $('#formImprimirAvaliacao').serialize(),
        type: "POST",
        success: function (dados, status, xhttp) {
            retornarAcaoImprimirHtml(dados);
        }
    });
}
