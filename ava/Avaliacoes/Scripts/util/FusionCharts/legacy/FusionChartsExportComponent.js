/**!
 * @license FusionCharts Export Component JavaScript Library
 ...
 *
 *  1.0.2 - 27/04/2009
 *  Added global debugMode to root object as well
 *  Added .version value
 *
 *  1.0.3 - 10/08/2010
 *  Removed infosoftglobal namespace dependency.
 *  Added v3.2 upscaling.
 *  
 *  1.0.4 - 06/01/2012
 *  Added ExportReady to advanced event model.
 *
 *  Notes:
 *  ======
 *
...
// Register the module with FusionCharts if needed.
...
_FCEO.defaultParameters = {
    ...
    exportAttributes: { },
    sourceCharts: [],
    debugMode: false, // set it to true to enable debugMode for ALL new objects
...
_FCEO.relayStream = function(streamObj)
{
	...
	// hung when the external script is heavy
        if (_FCEO.__global) {
            _FCEO.__global.raiseEvent('ExportReady', streamObj.parameters, 
                _FCEO.__global.core.items[streamObj.meta.DOMId] || _FCEO.__global.core);
...
_FCEO.updateComponent = function(obj) {
    ...
    _FCEO.L.sync(obj.componentAttributes, obj.srcObj.variables || obj.srcObj.flashVars);
    ...
    var getParam = function(val) {
        return !_FCEO.L.isObj((obj.srcObj.variables || obj.srcObj.flashVars)[val], 'undefined') ?
                (obj.srcObj.variables || obj.srcObj.flashVars)[val] : _FCEO.defaultParameters.componentAttributes[val];
    };

    try {
		// append mandatory flashVars
		(obj.srcObj.variables || obj.srcObj.flashVars)['generator'] = _FCEO.R.resSig;
		...
		if(prm != undefined) {
                    if (_FCEO.__global) {
                        obj.srcObj.width = prm.toString();
                        obj.srcObj.flashVars['width'] = prm.toString();
                    }
                    else {
		...
		if(prm != undefined) { 
                    if (_FCEO.__global) {
                        obj.srcObj.height = prm.toString();
                        obj.srcObj.flashVars['height'] = prm.toString();
                    }
                    else {
		...
		if(prm != undefined) {
			if (obj.srcObj.params) {
				obj.srcObj.params['bgColor'] = prm;
			}
			else {
