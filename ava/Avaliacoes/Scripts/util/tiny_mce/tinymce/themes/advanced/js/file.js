	...
	update : function(caminho) {
		...
		if (tinymce.isWebKit)
			...
			//ed.execCommand('mceInsertContent', 0, ed.dom.createHTML('a', {name : document.forms[0].anchorName.value, 'class' : 'mceItemAnchor'}, ''));
			  ed.execCommand('mceInsertContent', 0, '<a href="'+caminho+'">'+document.getElementById("narquivo").value+'</a>');
		tinyMCEPopup.close();
	}
};

tinyMCEPopup.onInit.add(File.init, File);
