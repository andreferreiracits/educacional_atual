...
<script src="../novoCombo/js/jquery.js"></script>
<script>
...
function carregaValsInput() {
		...
		var the_attr = ed.dom.getAttrib(the_id,'the_vals');
		//the_attr = the_attr.replace(/\+/g,'%2b');
		//alert(the_attr);
		var the_arr = tinymce.explode(the_attr,'|')
...
function insereValsInput() {
	...
	tinymce.each(els,function(o){
		...
		if(o.type == 'text'){
			if (o.value != '' && o.value){
				if (strs == '')
					strs = 'input|'+tinymce.trim(o.value);
				else
...
#voador{
	width:25px; height:17px;position:absolute;right:10px;top:10px;background-color:#FBC05A;
}
	
</style>
</head>
<body onLoad="carregaValsInput();">
<img src="images/topo_input.jpg?a=1">
<div id="mce_educ_geral">
	
	<div class="the_tit">Resposta padrão:</div>
    <div class="the_caixa"><input type="text" maxlength=60 id="principal_input"></div>
    <div class="the_tit">Outras respostas válidas: <img src="images/inserir.jpg" id="adicionar" title="adicionar mais respostas" onClick="adiciona()" style="position:absolute;right:45%;"></div>
