...
function Realizacao(containerPrincipal, idFormRealizacao, idLimpar, idRevisar, idConferir, idVoltar, idAvancar, lstTipos, idAtual, fCarregar, fErro) {
    ...
    this.atualizarListaQuestao = function (dados) {
        ...
        rlz.$this.find("#tmplListaQuestoes").tmpl(eval(lista)).appendTo("#listaQuestoes");
    ...
    */
    ...
    this.montarConteudo = function (dados) {
        ...
        new DicaRealizacao(rlz.$this.find('div.SEC025_Criterios .areaDicas:first'), rlz.$this.find(this.formulario + ' #idProvaRealizacao').val(), rlz.$this.find(this.formulario + ' #txtQuestao').val());
    };
    ...
    this.aplicarBotoesDiscursivaManual = function () {
        ...
        try {
            //aplicar o tiny
            var formatTine = $.extend({}, formatRealizacao, {
    ...
    this.aplicarBotoesRedacao = function () {
        rlz.desabilitarBtn($(rlz.btnConferir), "btnVerde_desabilitado", rlz.conferirQuestao);
        $("#realizadortoolTip .corpo").text("Essa questão será corrigida manualmente. Aguarde a correção para saber se respondeu corretamente.");
        $(".btnVerde_desabilitado").mouseover(function () {
            $("#realizadortoolTip").removeClass("hide");
        ...
        this.$this.find("#txtTextoRedacao").keyup(function () {
            if ($(this).val().length > 0) {
                rlz.habilitarBtn($(rlz.btnLimpar), "btnCinza_desabilitado", rlz.limparQuestao);
            } else {
                rlz.desabilitarBtn($(rlz.btnLimpar), "btnCinza_desabilitado", rlz.limparQuestao);
    ...
    this.aplicarBotoesLacunas = function () {
		...
		// Tratar questões antigas que não possuem as classes mceInput e mceCombo
		var $element = $("#lacuna_input_" + pos + "");
		if ($element.prop("tagName") == "INPUT") {
		    $element.addClass("mceInput");
		} else {
		    $element.addClass("mceCombo");
		}
    ...
    this.carregaInputLetraAssociar = function () {
        $("input[name=txtLetraAsso]").focusout(
            function (e) {
    ...
    this.carregaDragDrop = function () {
        ...
        $(".boxRecebeAssociado").droppable({
            ...
            drop: function (event, ui) {
                $(this).find("input[name=txtResposta]").val(ui.draggable.attr("id"));
                $(this).find("input[name=txtLetraAsso]").val($.trim(ui.draggable.text()));
...
function DicaRealizacao(container, idProva, posQuestao, idAlternativa) {
    ...
    this.tipos = ["Dica", "Comentario", "Professor", "Sugestao", "Correcao", "Criterios", "Eliminacao"];
    this.titles = {
        "Dica": "Dica",
        "Comentario": "Comentário",
        "Professor": "Sugestão para professor",
        "Sugestao": "Sugestão",
        "Correcao": "Correção",
        "Criterios": "Critérios",
    ...
    this.init = function () {
        for (var i = 0; i < this.tipos.length; i++) {
            ...
            var box = $(this.container).find('.box' + nomeTipo);
            var btn = box.find(' a.btnDica');
            btn.attr('data-type', nomeTipo);
            if (btn.attr('title') === undefined) {
                btn.attr('title', this.titles[nomeTipo]);
            }
            ...
            box.click(function (ev) {
                dica.showDica($(this).find(' a.btnDica'));
                ev.stopPropagation();
            });
            ...
            this.trocarNome(nomeTipo, box.parents(".areaDicas"), ".box" + nomeTipo + " a.btnDica", ".conteudo" + nomeTipo, 1);

            this.btns[nomeTipo] = btn;

            var conta = box.parents(".areaDicas").find(".areaConteudoDica .conteudo" + nomeTipo);
