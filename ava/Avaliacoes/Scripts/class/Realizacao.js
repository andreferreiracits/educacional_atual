...
function Realizacao(containerPrincipal, idFormRealizacao, idLimpar, idRevisar, idConferir, idVoltar, idAvancar, lstTipos, idAtual, fCarregar, fErro) {
    ...
    this.carregar = function (dados) {
        ...
        this.$this.find(this.btnLimpar).attr('href', FUNCAO_VAZIA).click(function () {
            if ($(this).attr("class").indexOf("_desabilitado") == -1) {
                rlz.limparQuestao();
            }
        });

        this.$this.find(this.btnRevisar).attr('href', FUNCAO_VAZIA).click(function () {
            if ($(this).attr("class").indexOf("_desabilitado") == -1) {
                rlz.revisarQuestao();
            }
        });
        this.$this.find(this.btnConferir).attr('href', FUNCAO_VAZIA).click(function () {
            if ($(this).attr("class").indexOf("_desabilitado") == -1) {
                rlz.conferirQuestao();
            }
        });
    ...
    this.iniLista = function () {
        ...
        this.$this.find("#btnListaQuestao").attr('href', FUNCAO_VAZIA).click(this.exibirListaQuestao);
    }
    ...
    this.atualizarListaQuestao = function (dados) {
        ...
        rlz.$this.find('ul#listaQuestoes > li').each(function () {
            var a = $(this).find(".paginacaoBtn a");
            var link;
    ...
    */
    this.criarNavegacao = function () {
        this.$this.find(this.btnVoltar).attr('href', FUNCAO_VAZIA).click(function () { rlz.voltarQuestao(); });
        this.$this.find(this.btnAvancar).attr('href', FUNCAO_VAZIA).click(function () { rlz.avancarQuestao(); });
    }
    ...
    this.aplicarBotoesSomatoria = function () {
        ...
        $('div#areaQuestao #alternativas li').each(function () {
            new DicaRealizacao($(this).find('.areaDica'), rlz.$this.find(rlz.formulario + ' #idProvaRealizacao').val(), rlz.$this.find(rlz.formulario + ' #txtQuestao').val(), $(this).find('input[type=hidden]').val());
    ...
    this.aplicarBotoesMultiplaEscolha = function () {
        ...
        $('div#areaQuestao #alternativas li').each(function () {
            ...
            new DicaRealizacao($(this).find('.areaDica'), rlz.$this.find(rlz.formulario + ' #idProvaRealizacao').val(), rlz.$this.find(rlz.formulario + ' #txtQuestao').val(), $(this).find('input[type=checkbox]').val());
    ...
    this.aplicarBotoesVerdadeiroFalso = function () {
        ...
        $('div#areaQuestao #alternativas li').each(function () {
            ...
            new DicaRealizacao($(this).find('.areaDica'), rlz.$this.find(rlz.formulario + ' #idProvaRealizacao').val(), rlz.$this.find(rlz.formulario + ' #txtQuestao').val(), $(this).find('input[name=idAlternativa]').val());
...
function DicaRealizacao(container, idProva, posQuestao, idAlternativa) {
    ...
    this.init = function () {
        ...
        for (var i = 0; i < this.tipos.length; i++) {
            ...
            var btn = $(this.container).find('.box' + nomeTipo + ' a.btnDica');
            btn.attr('title', nomeTipo);

            if (btn.length <= 0)
                continue;

            this.caminhos[nomeTipo] = btn.attr('href');

            btn.attr('href', FUNCAO_VAZIA).click(function () { dica.showDica($(this)) });
            this.textosComentario[nomeTipo] = this.tratarNome($(btn).html());

            this.trocarNome(nomeTipo, btn.parents(".areaDicas"), ".box" + nomeTipo + " a.btnDica", ".conteudo" + nomeTipo, 1);

            this.btns[nomeTipo] = btn;
        }
    ...
    this.showDica = function (btn) {
        ...
        if ($.trim(conta.find("div.textoDica").html()).length <= 26) {
            tst = $.ajax({
                ...
                type: "POST",
                success: function (dados, status, xhttp) { dica.retornoCarregar(conta.find("div.textoDica"), dados); }
            });
        }
    ...
    this.retornoCarregar = function (conta, dados) {
