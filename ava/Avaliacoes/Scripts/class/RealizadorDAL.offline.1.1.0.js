function RealizadorDAL (sCaminhoBase){
    ...
    this.objQuestaoResolvidaAtiva = {};

    this.init = function (fRetorno) {
        rdal.objQuestaoResolvidaAtiva = {};
        if (rdal.bolInit) {
            fRetorno();
            return;
        }
        ...
        this._LoadRealizacao(idAplicacao, function (sucess) {
            ...
            if (sucess || rdal.realizacao != null) {
                ...
                rdal._createRealizacao(idAplicacao, function (status) {
                    if (status.sucess) {
                        rdal.Realizar(idAplicacao, 0, fRetorno);
                    } else {
                        fRetorno(status);
                    }
                });
        ...
        if (bolOnline) {
            rdal.Online(function (status) {
                ...
                $.ajax({
                    ...
                    error: function (err1, err2, err3) {
                        rdal._novoDebugAlert("ListaAgendamentoProva ", { dados: dadosSend }, rdal.LOG_DEBUG)
    ...
    */
    ...
    this._MontRespostaSinc = function (count, form, fEnd) {
        rdal._novoDebugAlert("this._MontRespostaSinc", form.html(), rdal.LOG_WARN);
        ...
        for (var i = 0; i < rdal.realizacao.questoes.length; i++) {
            listaQuestoes.push(rdal._ComandoQuestaoResolvida(rdal.realizacao.questoes[i].id, rdal.idRealizada));
        }

        dbGet_array(listaQuestoes, function (retorno) {
            for (var i = 0; i < retorno.result.length; i++) {
                if (retorno.result[i] == "ok") {
                    var objResposta = retorno.data[i].situacao;
                    var pos = rdal.realizacao.questoes[i].i;
                    var rv = rdal.realizacao.questoes[i].rv;
                    var id = rdal.realizacao.questoes[i].id;
                    if (objResposta) {
                        var questaoJson = {
                    ...
                    rdal._novoDebugAlert("_MontRespostaSinc questao não encontrou objeto resposta", i, rdal.LOG_WARN);
                }
            }
            rdal._novoDebugAlert("form sincronia", form.html(), rdal.LOG_WARN);
            fEnd(form);
        });
        ...
        rdal._SaveRealizacao(idAplicacao, function (sucess) {
            if (sucess) {
                ...
                for (var i = 0; i < rdal.realizacao.questoes.length; i++) {
                    ...
                    listSinc.push(rdal._ComandoSalvarQuestaoResolvida(id, objResposta));
                }
                rdal._SaveComandos(listSinc, function (sucess) {
                    fRetorno(sucess);
                });
            } else {
                fRetorno(false);
    ...
    */
    this.QuestaoResponder = function (resposta, questaoAtual, fRetorno) {
        rdal._GetQuestaoResolvida(questaoAtual, function (id, objResposta) {
            ...
            if (!objResposta.resposta || objResposta.resposta == "")
            ...
            var lstComandos = [];
            lstComandos.push(rdal._ComandoSaveRealizacao(idAplicacao));
            objResposta = rdal._UpdateDate(objResposta);
            rdal.objQuestaoResolvidaAtiva = { id: id, valor: objResposta };
            lstComandos.push(rdal._ComandoSalvarQuestaoResolvida(id,objResposta));

            rdal._SaveComandos(lstComandos,function(sucess){
                if (sucess) {
...
//        
//        //if (rdal.objQuestaoResolvidaAtiva.valor && realizacao.questoes[pos].id == rdal.objQuestaoResolvidaAtiva.id) {
//        if (rdal.objQuestaoResolvidaAtiva.valor) {
//            fRetorno(rdal.objQuestaoResolvidaAtiva.id, rdal.objQuestaoResolvidaAtiva.valor);
//            return;
//        }
        ...
        dbGet("qr", id, idRealizada, function (data) {
            rdal._novoDebugAlert("_GetQuestaoResolvida", data, rdal.LOG_DEBUG);
            rdal.objQuestaoResolvidaAtiva = { id: id, valor: data.situacao };
    ...
    this._SalvarQuestaoResolvida = function (id, objResposta, fRetorno) {
        ...
        dbSet("qr", id, rdal.idRealizada, objResposta, function (data) {
            rdal._novoDebugAlert("_SalvarQuestaoResolvida", data, rdal.LOG_DEBUG);
            fRetorno(true);
        }, function (data) {
    ...
    /* novas formas de salvar */
    this._ComandoSaveRealizacao = function (idAplicacao) {
        var comando = [];
        comando[0] = "r";
        comando[1] = rdal.idUsuario;
        comando[2] = idAplicacao;
        comando[3] = rdal.realizacao;
        return comando;
    };

    this._ComandoQuestaoResolvida = function (id) {
        var comando = [];
        comando[0] = "qr";
        comando[1] = id;
        comando[2] = rdal.idRealizada;
        return comando;
    }

    this._ComandoSalvarQuestaoResolvida = function (id, objResposta) {

        var comando = [];
        comando[0] = "qr";
        comando[1] = id;
        comando[2] = rdal.idRealizada;
        comando[3] = objResposta;
        return comando;

    };

    this._SaveComandos = function (comandos, fRetorno) {
        rdal._novoDebugAlert("_SaveComandos", comandos, rdal.LOG_DEBUG);
        dbSet_array(comandos, function (retorno) {
            rdal._novoDebugAlert("_SaveComandos_retorno", retorno, rdal.LOG_DEBUG);
            for (var i = 0; i < retorno.result.length; i++) {
                if (retorno.result[i] != "ok") {
                    fRetorno(false);
                    return;
