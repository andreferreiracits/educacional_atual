...
(function (P, $) {
    ...
    P.add("avl_convert_old", function () {
        ...
        this.click = function ($a) {
            ...
            //recupera os dados para enviar
            var formSalvar = $('#avl_stps_config');
            var jsonSend = P.serializeObject(formSalvar);
            var entidade = formSalvar.data("avl_formjson-entidade");
            var auxiliares = $(formSalvar.data("element-auxiliar"));
            for (var i = 0, j = auxiliares.length; i < j; i++) {
                $.extend(jsonSend[entidade], P.serializeObject($(auxiliares[i]))[entidade]);
            }

            jsonSend.Avaliacao.Tipo = 4; //só vai salvar, não irá realizar a mudança, por enquanto
            P.ajax({
                url: formSalvar.attr("action"),
                data: JSON.stringify(jsonSend),
                type: formSalvar.attr("method"),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
