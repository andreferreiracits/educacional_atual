...
<ol>
    <li id="tourStart">
        ...
        <p>Bem-vindo ao Editor de Provas, uma ferramenta on-line utilizada por milhares de professores e alunos. A área de trabalho está organizada em abas.</p><a href='javascript:tour.startTour()'>Faça um tour »</a>
    </li>
    <li id="menuTour">
        ...
        <select class="tourselect" onchange="tour.gotoStep(this.value)">
              ...
              <option value="roteiro3">provas</option>
    ...
    <li id="roteiro1" title="Visão Geral">
        ...
        <ol>
            ...
            <li id="step2">
                ...
                A aba Provas exibe as suas provas e pesquisas, as compartilhadas pelos seus colegas de escola ou criadas pelo <%=this.TextosRecursos()["TextoPortal_"+ this.Usuario().TipoPortal.Id]%>.
            </li>
            <li id="step3">
                <input type="hidden" name="config" value='{"container":"#menuConteudo .abaAplicacao", "posicao":"LT", "url":"/Agendamento"}' />
                Depois de criar uma prova, você pode agendá-la definindo quem deve respondê-la e em qual período.
            </li>
            <li id="step4">
                <input type="hidden" name="config" value='{"container":"#menuConteudo .abaCorrecao", "posicao":"LT", "url":"/Correcao"}' />
                Se a prova tiver questões discursivas, você pode corrigi-las aqui.
            </li>
        </ol>
    </li>

    <li id="roteiro2" title="Questões">
        <input type="hidden" name="config" value='{"tipo":"roteiro", "urlbase":"<%=UtilView.Url("")%>", "urlroteiro":"<%=UtilView.Url("/TourRoteiros/questoes_pp.htm")%>"}' />
    </li>

    <li id="roteiro3" title="Provas">
        <input type="hidden" name="config" value='{"tipo":"roteiro", "urlbase":"<%=UtilView.Url("")%>", "urlroteiro":"<%=UtilView.Url("/TourRoteiros/avaliacoes_pp.htm")%>"}' />
    </li>

    <li id="roteiro4" title="Agendamentos">
        <input type="hidden" name="config" value='{"tipo":"roteiro", "urlbase":"<%=UtilView.Url("")%>", "urlroteiro":"<%=UtilView.Url("/TourRoteiros/agendamentos_pp.htm")%>"}' />
    </li>

    <li id="roteiro5" title="Correção">
        <input type="hidden" name="config" value='{"tipo":"roteiro", "urlbase":"<%=UtilView.Url("")%>", "urlroteiro":"<%=UtilView.Url("/TourRoteiros/correcao_pp.htm")%>"}' />
    </li>

    <li id="roteiro6" title="A Responder">
        <input type="hidden" name="config" value='{"tipo":"roteiro", "urlbase":"<%=UtilView.Url("")%>", "urlroteiro":"<%=UtilView.Url("/TourRoteiros/aresponder_pp.htm")%>"}' />
    </li>
