...
%>
    <li class="no <%=no.Editavel ? "editavel pertence":"" %> <%=no.RepresentaRaiz ? "representaraiz" : "" %>">
        <% 
        if (no.ERaiz)
        {
                %>
        ...
        <span class="nome">
            ...
            <span class="botoes">
                ...
                <%=Html.ActionLink("excluir", "ExcluirClassificacao", "NivelEnsino", new { @class = "btnExcluir btnExcluirNo" })%>
            </span>
            <% } %>
        </span>
        <% if (no.Editavel)
        ...
        <span class="edicao">
            ...
            <%=Html.ActionLink("salvar", "SalvarClassificacao", "NivelEnsino", new { @class = "btn btnSalvarNo" })%>
            <a class="btn btnCancelarNo">cancelar</a>
        </span>
        <% } %>
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Classificacao/NivelEnsino/ArvoreEditavel", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel, no.Id));
        %>
...
<% if (Model.Id == 0)
    {
            ...
            <li class="no">
                ...
                <a class="btn addno">
                    <span class="ico"></span>
                    <span>adicionar nível de ensino</span>
                </a>
            </li>
            <%
    } else{%>
