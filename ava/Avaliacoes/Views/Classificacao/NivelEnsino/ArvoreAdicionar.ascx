...
%>
    <li id="n_nivelEnsino_<%=no.Id %>" class="no <%=no.RepresentaRaiz ? "representaraiz" : "" %>">
        ...
        {
                %>
                <input id="chkFolha_n_nivelEnsino_<%= no.Ids %>" class="selecionar ico" type="checkbox" name="chkClassificacao_<%=EnumTipoClassificacao.NivelEnsino.Id %>" value="<%= no.Id %>">
                <%
        }
        ...
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Classificacao/NivelEnsino/ArvoreAdicionar", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel));
        %>
