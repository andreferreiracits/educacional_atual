...
%>
    <li class="no">
        ...
        {
                %>
                <input id="chkFolha_n_ebem_<%= no.Ids %>" class="selecionar ico" type="checkbox" name="chkClassificacao_<%=EnumTipoClassificacao.Enem.Id %>" value="<%= no.Id %>;<%= Html.Encode(itemNo.NomeCompleto) %>">
                <%
        }
        ...
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Classificacao/Enem/ArvoreFiltro", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel));
        %>
