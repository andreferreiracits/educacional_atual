...
%>
    <li class="no">
        ...
        {
                %>
                <input id="chkFolha_n_provinhaBrasil_<%= no.Ids %>" class="selecionar ico" type="checkbox" name="chkClassificacao_<%=EnumTipoClassificacao.ProvinhaBrasil.Id %>" value="<%= no.Id %>">
                <%
        }
        ...
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Classificacao/ProvinhaBrasil/ArvoreAdicionar", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel));
        %>
