...
%>
    <li id="n_areaAssunto_<%=no.Id %>" class="no">
        ...
        {
                %>
                <input id="chkFolha_n_areaAssunto_<%= no.Ids %>" class="selecionar ico" type="checkbox" name="chkClassificacao_<%=EnumTipoClassificacao.Assunto.Id %>" value="<%= no.Id %>">
                <%
        }
        ...
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Classificacao/Assunto/ArvoreAdicionarCriterio", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel));
        %>
