    ...
    <li class="no <%=no.Editavel ? "editavel pertence":"" %>">
        ...
        <span class="nome">
            ...
            <span class="botoes">
                ...
                <%=Html.ActionLink("excluir", "ExcluirClassificacao", "AreaAssunto", new { @class = "btnExcluir btnExcluirNo" })%>
            </span>
            <% } %>
        </span>
        <% if (no.Editavel)
        ...
        <span class="edicao">
            ...
            <%=Html.ActionLink("salvar", "SalvarClassificacao", "AreaAssunto", new { @class = "btn btnSalvarNo" })%>
            <a class="btn btnCancelarNo">cancelar</a>
        </span>
        <% } %>
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Classificacao/Assunto/ArvoreEditavel", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel, no.Id));
        %>
