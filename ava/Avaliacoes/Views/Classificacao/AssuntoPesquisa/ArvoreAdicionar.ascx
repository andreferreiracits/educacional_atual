...
%>
    <li id="n_areaAssunto_<%=no.Id %>" class="no">
        ...
        {
                %>
                <input id="chkFolha_n_areaAssuntoPesquisa_<%= no.Ids %>" class="selecionar ico" type="checkbox" name="chkClassificacao_<%=EnumTipoClassificacao.AssuntoPesquisaOpniao.Id %>" value="<%= no.Id %>">
                <%
        }
        ...
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Classificacao/AssuntoPesquisa/ArvoreAdicionar", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel));
        %>
