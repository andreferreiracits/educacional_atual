...
<%
    ...
    {
        foreach (AssuntoPesquisaOpniao assunt in Model.Linhas)
        {
%>
        <tr>
            <td width="50%">
                <a class="lnk"><%= assunt.Area.Nome %></a>
                <div class="botoes">
                    <%= Html.ActionLink("Excluir", "ExcluirTipoClassificacao", new { @id = assunt.Id }, new { @class = "btnExcluir" })%>
                </div>
            </td>
            <td width="50%"><%= assunt.Nome%></td>
        </tr>
<%
        }
    }else{
