...
%>
    <li id="n_nivelEnsino_<%=no.Id %>" class="no <%=no.RepresentaRaiz ? "representaraiz" : "" %>">
        ...
        {
                %>
                <input id="chkFolha_n_nivelEnsino_<%= no.Ids %>" class="selecionar ico notsend" type="checkbox" name="chkClassificacao_<%=EnumTipoClassificacao.Assunto.Id %>" value="<%= no.Id %>;<%= Html.Encode(no.NomeCompleto) %>">
        ...
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Questoes/Classificacao/NivelEnsino/Novo/ArvoreFiltro", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel));
        %>
