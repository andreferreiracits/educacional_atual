...
%>
    <li class="no">
        ...
        {
                %>
                <input id="chkFolha_n_habilidadeCompetencia_<%= no.Ids %>" class="selecionar ico" type="checkbox" name="chkClassificacao_<%=EnumTipoClassificacao.Habilidade.Id %>" value="<%= no.Id %>;<%= Html.Encode(noH.NomeCompleto) %>">
                <%
        }
        ...
        <% 
            if (no.TemFilhos)
                Html.RenderPartial("../Questoes/Classificacao/Habilidade/Novo/ArvoreFiltro", new Arvore<NoVO>(no.Filhos, Model.Editavel, Model.Selecionavel));
        %>
