...
<%	
	...
	'
	strLogin = Request("h")
	strLoginSuporte = Request("d")
    strSenhaSuporte = Request("n")
    sessionMan_sessionID = Request("g")
    SessionMan.Valor("suporte") = 1
    sessionMan.Valor("igorDebug") = "1"
    'response.Cookies("SessionMan_SessionId") = sessionMan_sessionID
...
'SessionMan.Valor("Escola_Dominio") = "colpositivo"
    ...
    'Verificação de Acesso a login Suporte
    ' Igor 12/04/2012
    Set oRSUsuario = oUsuario.Validar(strLoginSuporte, strSenhaSuporte) 'Login e senha do suporte
	If not oRSUsuario.BOF And not oRSUsuario.EOF Then 'Confere se este possui o papel de suporte				
		...
		If not oRSUsuario.BOF And not oRSUsuario.EOF Then
			...
			oRSUsuario.filter = "bolexcluido = 0" 'usuário não pode ser excluído 
			If not oRSUsuario.BOF And not oRSUsuario.EOF Then
...
'	end if
	...
	'************** SETA BOLCPPURO ********************	
	PATH_REDE_SOCIAL = "file name=D:\Componentes\redesocial.udl"
	spName = "redesocial.dbo.spAVA_VerificaCPPuro"	
	set rsBOLCP = spConnRS(spName, PATH_REDE_SOCIAL, Array(Array("idEscola",clng(idEscola),4)))	
		   
	if not rsBOLCP.eof then
        if rsBOLCP("bolCPPuro") Then
		    SessionMan.Valor("bolCPPuro") = 1
        else
            SessionMan.Valor("bolCPPuro") = 0
        end if
    else
        SessionMan.Valor("bolCPPuro") = 0
	end if
