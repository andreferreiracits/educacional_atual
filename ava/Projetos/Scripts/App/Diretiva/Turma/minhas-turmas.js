"use strict"
angular.module('Turma').directive('minhasTurmas', function () {
    return {
        restrict: 'E',
        scope: {
            edicao: '=edicao',
            usuario: '=usuario',
            turmas: '=turmas',
            defaultConfig: '=defaultConfig',
            listaAnos : "=listaAnos"
        },
        templateUrl: '/AVA/Projetos/Scripts/App/Diretiva/Turma/minhas-turmas.html',
        controller: ['$http', '$scope', '$timeout', function ($http, $scope, $timeout) {
            var _self = this;

            $scope.filtroano = 0;
            this.filtroSituacao = 0;
            this.filtroEtapa = 0;
            this.filtroTurma = 0;
            this.intPaginaAtual = 1;

            $scope.convCatToClas = function (nomeCategoria) {

                nomeCategoria = nomeCategoria.toLowerCase();
                nomeCategoria = nomeCategoria.replace(/ç/g, 'c');
                nomeCategoria = nomeCategoria.replace(/ã|á|à|â|ä|â/g, 'a');
                nomeCategoria = nomeCategoria.replace(/é|ê|è|ë/g, 'e')
                nomeCategoria = nomeCategoria.replace(/í|ì|î|ï/g, 'i');
                nomeCategoria = nomeCategoria.replace(/õ|ô|ó|ò|ö/g, 'o');
                nomeCategoria = nomeCategoria.replace(/û|ú|ù|ü/g, 'u');
                nomeCategoria = nomeCategoria.replace(/[^\w\s]/gi, '');
                nomeCategoria = nomeCategoria.replace(/\s+/g, "-");
                return nomeCategoria;

            };
            
            this.objEnviosInscricao = {
                totalEnvios: 0,
                envios: []
            };

            this.detalheTurma = {
                Id: 0
            };

            this.loadingBuscarGeral = true;

            this.etapaAberta = function(et) {
                var bolEtapaAberta = true;

                try {                                        
                    var intDiferenca = new Date() - new Date(parseInt(et.DataInicio.replace(/\D+/g, "")));

                    if (intDiferenca < 0) {
                        bolEtapaAberta = false;
                    }
                }
                catch(err) {
                    
                };

                return bolEtapaAberta;
            };

            this.getLinkEtapa = function(envio) {
                if (envio.SituacaoEnvio.Id == 2) {
                    return "";
                }

                var retorno = "/AVA/Projetos/" +  new Date(parseInt(envio.DataCriacao.substr(6), 10)).getFullYear() + "/" + $scope.edicao.Link + "/Etapas/" + envio.Etapa.Link + "/";
                
                if (envio.MensagemRapida && envio.MensagemRapida.StrEncryptIdMensagem && envio.MensagemRapida.StrEncryptIdMensagem != "") {
                    retorno += envio.MensagemRapida.StrEncryptIdMensagem;
                }
                else {
                    retorno += "Formulario";
                }

                return retorno;
            };

            this.filtraSituacao = function(idSituacao) {
                if (_self.filtroSituacao != idSituacao && !_self.loadingBuscarGeral) {
                    _self.filtroSituacao = idSituacao;
                    _self.aplicaFiltros();
                }                                
            };
            
            this.aplicaFiltros = function() {
                _self.loadingBuscarGeral = true;
                _self.intPaginaAtual = 1;

                _self.objEnviosInscricao.envios = [];
                _self.objEnviosInscricao.totalEnvios = 0;
                
                if (_self.filtroTurma > 0 && _self.filtroTurma !== _self.detalheTurma.Id) {
                    var intFiltroTurma = parseInt(_self.filtroTurma);
                    for (i = 0; i < $scope.turmas.length; i++) {
                        if ($scope.turmas[i].Id === intFiltroTurma) {
                            _self.detalheTurma = $scope.turmas[i];
                            _self.detalheTurma.linkMapa = "https://maps.google.com/maps?q=" + $scope.usuario.Escola.Cidade + "," + $scope.usuario.Escola.Estado + ",Brasil&ie=UTF8&hq=&hnear=" + $scope.usuario.Escola.Cidade + "&t=m&z=9&iwloc=A&output=embed";
                            
                            $http({
                                url: "/AVA/Projetos/Servico/BuscarTurmaAlunosById",
                                method: "POST",
                                params: {
                                    idTurma: _self.filtroTurma
                                }
                            }).success(function(data) {
                                _self.detalheTurma.Alunos = data.alunos;
                            }).finally(function() {
                                $http({
                                    url: "/AVA/Projetos/Servico/BuscarInscricaoByTurmaEdicao",
                                    method: "POST",
                                    params: {
                                        idTurma: _self.filtroTurma,
                                        idProjetoEdicao: $scope.edicao.Id
                                    }
                                }).success(function(data) {
                                    _self.detalheTurma.Inscricao = data.inscricao;
                                }).finally(function() {
                                    _self.carregaConteudo();
                                });
                            });
                        }
                    }
                }
                else {
                    _self.carregaConteudo();
                }
            };

            this.carregarMais = function() {
                if (!_self.loadingBuscarGeral) {
                    _self.loadingBuscarGeral = true;
                    _self.intPaginaAtual++;

                    _self.carregaConteudo();
                }                
            };

            this.carregaConteudo = function () {
                console.log(_self.filtroSituacao);
                $http({
                    //url: '/AVA/Projetos/Servico/GetMeusEnviosPaginado/',
                    url: '/AVA/Projetos/Servico/GetMeusEnviosPaginadoAndPreAprov/',
                    method: 'POST',
                    params: {
                        idProjeto: $scope.edicao.Projeto.Id,
                        idProjetoEdicao: $scope.edicao.Id,
                        idProjetoEdicaoEtapa: _self.filtroEtapa,
                        idSituacao: _self.filtroSituacao,
                        idTurma: _self.filtroTurma,
                        tipoOrdenacao: 2,
                        intPagina: _self.intPaginaAtual,
                        intRegPorPagina: 6,
                        intAno : $scope.filtroano
                    }
                }).success(function (data) {
                    if (data && data instanceof Object) {
                        if (data.TotalEnvios && !isNaN(data.TotalEnvios) && parseInt(data.TotalEnvios) >= 0) {
                            _self.objEnviosInscricao.totalEnvios = parseInt(data.TotalEnvios);
                        }

                        if (data.listParticipanteEnvio && data.listParticipanteEnvio instanceof Array && data.listParticipanteEnvio.length > 0) {
                            if (_self.objEnviosInscricao.envios.length == 0) {
                                _self.objEnviosInscricao.envios = data.listParticipanteEnvio;
                            }
                            else {
                                angular.forEach(data.listParticipanteEnvio, function (valor, chave) {
                                    _self.objEnviosInscricao.envios.push(valor);
                                });
                            }

                            for (var i = 0; i < _self.objEnviosInscricao.envios.length; i++) {
                                if (_self.objEnviosInscricao.envios[i].SituacaoEnvio.Descricao.toLowerCase() === 'pré-aprovado' || _self.objEnviosInscricao.envios[i].SituacaoEnvio.Descricao.toLowerCase() === 'aguardando publicação') {
                                    _self.objEnviosInscricao.envios[i].SituacaoEnvio.Descricao = 'Em Moderação';
                                }
                            }

                            if (data.listParticipanteEnvio.length < $scope.maxRegistroPagina) {
                                $scope.bolFimMateriais = true;
                            }
                            else {
                                $scope.bolFimMateriais = false;
                            }
                        }
                        else {
                            $scope.bolFimMateriais = true;
                        }
                    }
                }).error(function (err) {
                    console.log("Não foi possível buscar materiais da etapa");
                }).finally(function(){
                    _self.loadingBuscarGeral = false;
                    //_self.carregaEnvModeracao();
                });
                
            };
            /*
            // Esta funcao foi adicionada para inclusao do redmine issue #95995 - O status "pre-aprovado" nao estava sendo filtrado em nenhuma das requisições
            // aqui estamos chamando manualmente a busca destes estatus (caso existam) nas abas 'todos' e 'moderação'. essa patch foi necessaria pois
            // em uma mesma aba (moderação) devem ser apresentados itens com status diferentes, 'novo' e 'pre-aprovado'
            this.carregaEnvModeracao = function () {
                if (_self.filtroSituacao == 0 || _self.filtroSituacao == 2) {
                    if (!$scope.filtroano || $scope.filtroano <=0) {
                        $scope.filtroano = new Date().getFullYear();
                    }
                    $http({
                        url: '/AVA/Projetos/Servico/GetMeusEnviosPaginado/',
                        method: 'POST',
                        params: {
                            idProjeto: $scope.edicao.Projeto.Id,
                            idProjetoEdicao: $scope.edicao.Id,
                            idProjetoEdicaoEtapa: _self.filtroEtapa,
                            idSituacao: 5,
                            idTurma: _self.filtroTurma,
                            tipoOrdenacao: 2,
                            intPagina: _self.intPaginaAtual,
                            intRegPorPagina: 6,
                            intAno: $scope.filtroano
                        }
                    }).success(function (data) {
                        if (data && data instanceof Object) {
                            if (data.TotalEnvios && !isNaN(data.TotalEnvios) && parseInt(data.TotalEnvios) >= 0) {
                                if (_self.objEnviosInscricao.totalEnvios) {
                                    _self.objEnviosInscricao.totalEnvios = parseInt(data.TotalEnvios) + _self.objEnviosInscricao.totalEnvios;
                                } else {
                                    _self.objEnviosInscricao.totalEnvios = parseInt(data.TotalEnvios);
                                }
                            }

                            if (data.listParticipanteEnvio && data.listParticipanteEnvio instanceof Array && data.listParticipanteEnvio.length > 0) {
                                if (_self.objEnviosInscricao.envios.length == 0) {
                                    _self.objEnviosInscricao.envios = data.listParticipanteEnvio;

                                    for (var i = 0; i < _self.objEnviosInscricao.envios.length; i++) {
                                        if (_self.objEnviosInscricao.envios[i].SituacaoEnvio.Descricao === 'Pré-Aprovado' || _self.objEnviosInscricao.envios[i].SituacaoEnvio.Descricao === 'Pré-aprovado') {
                                            _self.objEnviosInscricao.envios[i].SituacaoEnvio.Descricao = 'Em Moderação';
                                        }
                                    }    
                                }
                                else {
                                    angular.forEach(data.listParticipanteEnvio, function (valor, chave) {
                                        if (valor.SituacaoEnvio.Descricao === 'Pré-Aprovado' || valor.SituacaoEnvio.Descricao === 'Pré-aprovado') {
                                            valor.SituacaoEnvio.Descricao = 'Em Moderação';
                                        }
                                        _self.objEnviosInscricao.envios.push(valor);
                                    });
                                }

                                if (data.listParticipanteEnvio.length < $scope.maxRegistroPagina) {
                                    $scope.bolFimMateriais = true;
                                }
                                else {
                                    $scope.bolFimMateriais = false;
                                }
                            }
                            else {
                                $scope.bolFimMateriais = true;
                            }
                        }
                    }).error(function (err) {
                        console.log("Não foi possível buscar materiais da etapa");
                    }).finally(function () {
                        _self.loadingBuscarGeral = false;
                    });
                }
            };
            //start above
            */
            
            if($scope.listaAnos && $scope.listaAnos instanceof Array) if($scope.listaAnos.length > 1){
                if($scope.listaAnos[$scope.listaAnos.length - 1] > 0){
                    $scope.filtroano = parseInt($scope.listaAnos[$scope.listaAnos.length - 1]);
                }
            }

            if($scope.edicao.BolEncerrado){
                _self.filtroSituacao = 1;
            }
            
            this.carregaConteudo();
            
        }],
        controllerAs: 'ctrlMinhasTurmas'
    };
});