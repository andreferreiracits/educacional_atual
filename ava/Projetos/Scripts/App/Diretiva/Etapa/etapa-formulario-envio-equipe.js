...
angular.module('Inscricao.mista')
    .directive('etapaFormularioEnvioEquipe', ['$http', '$document', 'EtapaFactory', '$timeout', function ($http, $document, $etapaFactory, $timeout) {
        return {
            restrict: 'E',
            //replace: true,
            templateUrl: '/AVA/Projetos/Scripts/App/Diretiva/Etapa/etapa-formulario-envio-equipe.html',
            scope: {
                equipe: "=equipe",
                objEtapa : "=objEtapa",
    ...
    angular.module('Inscricao.mista').controller('etapaFormularioEnvioEquipeCtrl', ['$scope', '$filter', '$timeout', '$http', 'EtapaFactory', function ($scope, $filter, $timeout, $http, $etapaFactory) {
        /*
        ...
        */
        ...
        $scope.enviarEtapaFormulario = function () {
            ...
            //{
            //    idInscricao: $scope.equipe.Inscricao.Id,
            ...
            .success(function (data) {
                if (data.incompleto != null) {
                    ...
                    if (confirm(msgConfirm)) {
                        self.finalizarEnvioFormulario($scope.equipe.Inscricao.Id, $scope.etapaConfig.Etapa.Id);
                    }
        ...
        this.finalizarEnvioFormulario = function (idInscricao, idEtapa) {
            ...
            //})
            var idEnvio = 0;
            if($scope.idx >= 0 && angular.isArray($scope.equipe.Inscricao.EtapaInscricaoEnvios) && angular.isObject($scope.equipe.Inscricao.EtapaInscricaoEnvios[$scope.idx]) && $scope.equipe.Inscricao.EtapaInscricaoEnvios[$scope.idx].Id>0)
                idEnvio = $scope.equipe.Inscricao.EtapaInscricaoEnvios[$scope.idx].Id;
            var parametros = {
                idInscricao: $scope.equipe.Inscricao.Id,
                idEtapa: $scope.etapaConfig.Etapa.Id,
                idEnvio: idEnvio,
            ...
            $http({
                ...
                if (data.successo != null) {
                    if (data.TurmaEnvio != null) {
                        $scope.equipe.Inscricao.EtapaInscricaoEnvios = data.TurmaEnvio;
        ...
        $scope.retornoSalvar = function (original_data) {
            if (original_data) if (original_data.retorno.idFormularioResposta > 0) {
                ...
                //        '_': new Date().getTime()
                //    },
                //    url: "/AVA/Projetos/Etapa/SalvarEtapaFormularioEquipeClube",
                //    method: 'POST'
                //})

                $http.post("/AVA/Projetos/Etapa/SalvarEtapaFormularioEquipeClube",
                {
                    idInscricao: $scope.equipe.Inscricao.Id,
                    idEdicao: $scope.objEtapa.Edicao.Id,
                    idEquipe: $scope.equipe.Id,
                    ...
                    if (data.Envio != null) {
                        $scope.equipe.Inscricao.EtapaInscricaoEnvios = data.Envio;

                        if ($scope.equipe.Situacao) {
                            if ($scope.equipe.Situacao.Id == 4) {
                                $scope.equipe.Situacao.Id = 1; //Seta equipe para situação publicada no primeiro envio!
                            }
                        }

                        $scope.equipe.TotalEnvios = data.TotalEnvios;
                        $scope.bloqueiaAllEquipe.block = true;
                        $scope.acesso.primeiro = false;

                        $scope.$emit("changeListaInscricoesEquipe", {
                            Envio: angular.copy(data.Envio),
                            TotalEnvios: angular.copy(data.TotalEnvios),
                            idFormularioResposta: angular.copy(original_data.retorno.idFormularioResposta),
        ...
        $scope.$on("setFormularioAlterado", function (event) {
            if ($scope.equipe.Inscricao.EtapaInscricaoEnvios != null) {
                $scope.equipe.Inscricao.EtapaInscricaoEnvios[$scope.idx].BolPodeEnviar = false;
            }
        });
        ...
        this.abrirFormularioOptativo = function (indexGrupo, indexMaterial, idGrupoForm, idFormResposta) {
            ...
            if ($scope.equipe.Inscricao.EtapaInscricaoEnvios != null && idFormResposta > 0) {
        ...
        this.getIdFormRespostaByOrdem = function (idx, idEtapaGrupo) {
            ...
            var objTurmaSelecionada = $scope.equipe;

            if (objTurmaSelecionada.Inscricao != null) {
                if (objTurmaSelecionada.Inscricao.EtapaInscricaoEnvios != null) {
                    if (objTurmaSelecionada.Inscricao.EtapaInscricaoEnvios[$scope.idx].EtapaInscricaoEnvioGrupos != null) {
                        var objGrupo = $filter('filter')(objTurmaSelecionada.Inscricao.EtapaInscricaoEnvios[$scope.idx].EtapaInscricaoEnvioGrupos, { EtapaGrupo: { Id: idEtapaGrupo} }, true);
