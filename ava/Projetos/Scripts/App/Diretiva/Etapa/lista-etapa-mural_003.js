"use strict"
angular.module('Etapa').directive('listaEtapaMural', function () {
    return {
        restrict: 'EA',
        templateUrl: '/AVA/Projetos/Scripts/App/Diretiva/Etapa/lista-etapa-mural.html',
        scope: {
            objEtapas: "=objEtapas",
            objEdicao: "=objEdicao"
        },
        ...
        controller: ['$http', '$scope', '$filter', '$modal', 'constantes', 'projetoTools', function ($http, $scope, $filter, $modal, constantes, projetoTools) {
            var self = this;
            ...
            $scope.hrefLinkEtapa = function (p_etapa) {
                ...
                switch (projetoTools.situacaoEtapa(p_etapa)) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        if ([1, 3, 5].indexOf(p_etapa.TipoEtapaEleicao) != -1 && $scope.bolUsuarioInscrito) {
                            //eleições
                    ...
                    case 4:
                        if ([1, 3, 5].indexOf(p_etapa.TipoEtapaEleicao) != -1) {
                            //eleições
                            if ([1, 3].indexOf(p_etapa.TipoEtapaEleicao) != -1) {
                                if ($scope.situacaoEtapaFinal != 4 && !$scope.bolMultiplosComites) {
                                    retorno = $scope.strLinkComite(p_etapa)
                                }
                            }
