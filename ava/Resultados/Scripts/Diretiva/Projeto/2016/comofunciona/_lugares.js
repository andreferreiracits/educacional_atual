...
angular.module("resultado").controller('ComoFuncionaLugares2016Ctrl', ["$http", "$scope", "$timeout", "$filter", "$location","projetoMapa", function ($http, $scope, $timeout, $filter, $location, projetoMapa) {
    var _self = this;
	...
	$scope.onDemandLoad=function(carousel)
	{
		jQuery('#da-slider').find('.midia').each( function(i) {
			if (i>=0 && i<carousel.options.scroll){
                ...
                */
				//jQuery(this).attr('src', $(this).attr('_src'));
			}
		});
		...
		carousel.buttonNext.bind('click', function() {
			...
			jQuery('#da-slider').find('.midia').each( function(i) {
				var first = 0;
				var last = carousel.options.scroll;
				if (carousel.last != 'undefined'){
					first = carousel.last;
					last = carousel.last+carousel.options.scroll;
				}
				if (i>=first && i<last){
                    angular.element(this).css({
	                    'background-image': 'url(' + angular.element(this).get(0).getAttribute("lazy_back_img") +')'
        ...
        $timeout(function(){
            ...
            if(true){
                ...
                var parametros = {
                    ...
                    intDestaque: 0,
                    intEtapaRelativa: 0,
                    idCategoria: $scope.categoriasEtapa1[1].Id,
                    termoBusca: ''
                };
                    ...
                    if (data) if (data instanceof Object) {
                        ...
                        if (data.listParticipanteEnvio) if (data.listParticipanteEnvio instanceof Array) if (data.listParticipanteEnvio.length > 0) {
                            if(bolAppend){
                                ...
                                $scope.safeApply();
                                $scope.$broadcast("updateCarousel", $scope.Envios);
                            }
                        }
