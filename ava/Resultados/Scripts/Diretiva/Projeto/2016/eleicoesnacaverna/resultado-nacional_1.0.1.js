...
angular.module("resultado").controller('Eleicoes2016Ctrl', ["$http", "$rootScope", "$scope", "$timeout", "$filter", "$location", "$state", "$modal", "projetoTools", "constantes", function ($http, $rootScope, $scope, $timeout, $filter, $location, $state, $modal, projetoTools, constantes) {
    var self = this;
        ...
        for (var cargo in cargos) {
            ...
            for (var candidato in tempCargo) {
                if (totalValidos > 0 && tempCargo[candidato].votos > 0) {
                    tempCargo[candidato].indiceCargo = cargos[cargo];
