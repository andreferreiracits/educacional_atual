...
angular.module('resultado').directive('jogoQuiz',['$http','$timeout','$filter', function ($http, $timeout, $filter) {
    return {
        ...
        templateUrl: function (element, attrs) {
            return '/AVA/Resultados/Scripts/Diretiva/Clube/sustentabilidade/quiz.html';
        },
        link: function link(scope, element, attrs) {
            ...
            scope.idResposta=0;
			scope.idxJogo=1;
            scope.idxAlternativaCorreta = undefined;
			scope.intPagina = 0;
			...
			scope.iniciarJogo=function(idxJogo,bolPreservaPontucao){
                scope.loadingRanking = true;
                scope.idxJogo=idxJogo;
                scope.configJogo = angular.copy(scope.CacheJogo[idxJogo -1].jogoConfig);
                scope.CacheQuestoes = angular.copy(scope.CacheJogo[idxJogo -1].listQuestoes);
				...
				if (scope.CacheJogo[idxJogo -1].listaRanking) if (scope.CacheJogo[idxJogo -1].listaRanking instanceof Array) if (scope.CacheJogo[idxJogo -1].listaRanking.length > 0) {
					scope.Ranking = scope.CacheJogo[idxJogo -1].listaRanking;
				}
			...
			scope.confereResposta=function(questao,alternativa){
				...
				if(bolProssegue){
					...
					if(acertou){
                        for(var alt in questao.Alternativas){
                            if(questao.Alternativas[alt].Id == alternativa.Id){
                                scope.idxAlternativaCorreta = questao.Alternativas.indexOf(questao.Alternativas[alt]);
                            }
                        } 
            ...
            scope.getBackImageAlternativa=function(questao,alternativa){
                if(scope.isImage(alternativa.Conteudo)){
                    if(questao.TempoPreview==0){
                        ...
                        if(scope.TempoPreviewRestante>0){
                            return '/AVA/StaticContent/Projetos/Projeto/2015/comofunciona/imagens/pergunta.jpg';
                        }
                        else{
                            if(scope.TempoPreviewRestante==0){
                                return alternativa.Conteudo
                ...
                */
				...
				$timeout(function(){
					$http({
                        ...
                        var data = data_new;
                        scope.CacheJogo = data_new;
                        if (data) if (data instanceof Array && scope.idxJogo>0) {
                            data = data_new[scope.idxJogo-1];
                        }
						if (data) if (data instanceof Object) {
							...
							/*if (data.listaRanking) if (data.listaRanking instanceof Array) if (data.listaRanking.length > 0) {
								if(bolAppend){
									for(var participante in data.listaRanking){
                                        console.log(data.listaRanking[participante]);
                            ...
                            if(resetJogo){
                                scope.iniciarJogo(scope.idxJogo,true);
                            }
						}
                        scope.loadingRanking = false;
						scope.loadInProgress = false;
...
angular.module('resultado').controller('SustentabilidadeJogoCtrl', 
