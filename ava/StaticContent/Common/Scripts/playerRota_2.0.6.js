...
jQuery(function ($) {
    ...
    /*******************
    * Força altura da div para não quebrar rodapé ID 18414
    ********************/
    var a = $(".etapa_infos:visible").height();
    $("#ava_box_player").height(a + 10);
    /*******************/
    ...
    ********************/
    ...
    /*Esconde as mensagens do forum*/
    $(".forum_mensagens").hide();
    ...
    ********************/
    if (pathname.toLowerCase().indexOf('/ava/caminhos/home/player') > -1) {
        $('.abrePlayer').each(function () {
            ...
            if ($_idEtapa == i_idEtapa) {
                ...
                if (idRecurso == "1" || idRecurso == "9" || idRecurso == "11" || idRecurso == "12" || idRecurso == "8") {
                ...
                if (trabalho.attr("class") != "container_entrega_aluno") {
                    if (cxMsg.is(":visible") && placa.hasClass("atual")) {
                        ...
                        setTimeout(function () {
                            ...
                            if (!estaPlaca.find(".sprite_player.bolConcluidoPlaca").hasClass("etapaForaDoPrazo")) {
    ...
    $(".abreSecao").live("click", function () {
        ...
        $_idEtapa = $(this).attr("idEtapa");
        if (location.href.toLowerCase().indexOf('rota=') > -1) {
        ...
        if (teste.attr("class") == "container_entrega_aluno") {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data != "0") {
                        if (!este.find(".sprite_player.bolConcluidoPlaca").hasClass("etapaForaDoPrazo"))
    ...
    $('.abrePlayer').live('click', function () {
        ...
        if (teste.attr("class") == "container_entrega_aluno") {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data != "0") {
                        if (!este.prev().children(":first").find(".sprite_player.bolConcluidoPlaca").hasClass("etapaForaDoPrazo"))
    ...
    ********************/
                ...
                success: function (data) {
                    ...
                    if (!isNaN(codigo[0])) {
                        if (codigo[0] != "0" && codigo[0] != "2") {
                            // Aqui deve procurar a caixa aberta de avaliação e pegar os parametros pra marca o check de concluido => .etapa_infos
                            if (!este.hasClass('etapaForaDoPrazo'))
                            ...
                            if (data == "0") {
                                ...
                                if (placa.parent().next(".etapa_infos").is(":visible")) {
                                    setTimeout(function () {
                                        ...
                                        if (!placa.find(".sprite_player.bolConcluidoPlaca").hasClass("etapaForaDoPrazo")) {
                                            salvarCaminhoEtapaUsuario(idRotaEtapaUsuario, idRotaUsuario, idEtapa, idRecurso, idRotaAgendamento, true, "normal", "nao");
                                            verificaTodosConcluidosParaEncerrarCaminho(idRotaUsuario);
    ...
    $(".sobedesce_material").click(function () {
        ...
        //carregaArquivosCaminho(idAuxRotaEtapaUsuario);
    });
});
...
function enviarTrabalhoMarcarConcluido(elemento) {
    ...
    if (!(elemento.closest(".etapa_infos").prev().children(":first").find(".sprite_player.bolConcluidoPlaca").hasClass("concluido")) &&
        !(elemento.closest(".etapa_infos").prev().children(":first").find(".sprite_player.bolConcluidoPlaca").hasClass("etapaForaDoPrazo"))) {
    ...
    $.ajax({
        ...
        data: {
            idRotaUsuario: idRotaUsuario,
            idEtapa: idEtapa //elemento.attr("idEtapa") //idEtapaAtual
        },
        async: true,
        cache: false,
        success: function (data) {
            if (!elemento.closest(".etapa_infos").prev().children(":first").find(".sprite_player.bolConcluidoPlaca").hasClass("etapaForaDoPrazo")) {
                if (data != "0") {
...
function enviarTrabalhoMarcarConcluidoFim(idEtapa, idRotaEtapaUsuario) {
    ...
    //var idRotaEtapaUsuario = elemento.attr("idRotaEtapaUsuario");
    //var idRotaEtapaUsuario = $("#idRotaEtapaUsuario_" + idEtapa).val();
                            ...
                            if (codigo[0] != "0" && codigo[0] != "2") {
                                ...
                                if (este.length > 0 && este.length == 1) {
                                    if (este.find(".avaliacao").hasClass("avaliacao")) {
                                        ...
                                        if (!(pAmarela.hasClass("concluido")) && !(pAmarela.hasClass("etapaForaDoPrazo"))) {
                                        ...
                                        $('.ava_abreAvaliacao.avaliacao').text('Visualizar avaliação');
                                    }
                                }
                                else {
                                    console.log(data);
...
function executaPlayer(e) {
    ...
    } else if (idRecursoPlayer == 2 || $rel.indexOf('linhadotempo') > 0) {
    ...
    else if (idRecursoPlayer == "10" || idRecursoPlayer == "5") {
...
function verConteudoFancy() {
    //faz a atividade
    $(".orientacoes_player").click();
...
function abrirFancy(){
    //conteudo do frame
    var iframe = $('#ifrmPlayer');
    var urlIframe  ="";
    if (iframe.length) {
        urlIframe = $('#ifrmPlayer').contents().find("body").find("iframe").attr("src")
    }
    if (urlIframe != "") {
        //abre em popUp
        $.fancybox.open({
...
function abreAvaliacao(e) {
    var idConfig = 0;
    var idAplicacao = e.attr("intordemagendamento"); // é o idAgendamento
    var opcoes = {
        'idAplicacao': idAplicacao,
        'idConfig': idConfig,
        'caminhoBase': '/AVA/Avaliacoes'
    };
    var idUsuario = idUsuarioAvaliacao;  // esta variavel é criada no arquivo playerRota
    $.fancybox({
        ...
        afterShow: function () {
            ...
            if (verificaCaminhoEstaEncerrado) { // esta variavel é criada no arquivo playerRota
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.erro && data.erro.id == 3) {
                            ...
                            $("#abrirAvalicaoBox").avaliacoesRealizacao(opcoes).avaliacoesRealizacao("realizar", { questao: 0 });
                            var alturaLightBox = $(".fancybox-skin").height();
                            $("#abrirAvalicaoBox").parent().css("height", alturaLightBox + "px");
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data != "Caminho já está encerrado" && data.substring(0, 200).indexOf("doctype html") < 0) {
                if (data != "-1") {
                    if (idRotaEtapaUsuarioAtual == "0" || idRotaEtapaUsuarioAtual == "-1" || (!(elAux.find("> a > .sprite_player.bolConcluidoPlaca").hasClass("concluido")) && !(elAux.find("> a > .sprite_player.bolConcluidoPlaca").hasClass("etapaForaDoPrazo")))) {
