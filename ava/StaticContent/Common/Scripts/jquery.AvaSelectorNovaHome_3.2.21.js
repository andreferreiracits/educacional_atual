(function ($) {
    var methods = {
        ...
        limparUsuarios: function () {
            return this.each(function () {
                ...
                if (data) {
                    ...
                    if (data.opcoes.caixaLajotaExterna == null) {
                        $('.seletor_lista .seletor_item', principal).remove();
                        if (data.opcoes.strLajotinhaTodos != null) {
                            var $auxLajTod = $(data.uteis.htmlLajotinhaTodos);
                            $('.seletor_lista', principal).prepend($auxLajTod);
                            $auxLajTod.qtip(data.uteis.qtipOptions);
                        }
                    } else {
                        $('.seletor_lista .seletor_item', data.opcoes.caixaLajotaExterna).remove();
                        if (data.opcoes.strLajotinhaTodos != null) {
                            var $auxLajTod = $(data.uteis.htmlLajotinhaTodos);
                            $('.seletor_lista', data.opcoes.caixaLajotaExterna).prepend($auxLajTod);
                            $auxLajTod.qtip(data.uteis.qtipOptions);
                        }
                    }
                } else {
        ...
        init: function (parametros) {
            console.log("bolCoordenador ********* " + $("#bolCoordenador").val())
            var opcoes = {
                "bolAluno": true,
                "bolProfessor": false,
                ...
                "bolCoordenador": $("#bolCoordenador").val(),//  true,
            ...
            return this.each(function () { //deve achar a div que foi colocado na pagina para fazer a chamada
                ...
                var htmlLajotinhaTodos = opcoes.strLajotinhaTodos == null ? '' : '<div class="seletor_item grupo todos" title="' + opcoes.strLajotinhaTodos + '"><span>Todos</span></div>';
                ...
                if (!data) {
                    if (!opcoes.bolAdminCoordDiretor && !opcoes.bolAluno && !opcoes.bolProfessor && !opcoes.bolResponsavel && !opcoes.bolSeguidores) {
                        ...
                        function acaoBotaoInscrever() {
                            if (opcoes.bolLajota) {
                                ...
                                if (opcoes.caixaLajotaExterna == null) {
                                    cxDestinoAppend = principal.find(".seletor_lista");
                                } else {
                                    cxDestinoAppend = opcoes.caixaLajotaExterna.find(".seletor_lista");
                                }

                                $('.seletor_item', cxDestinoAppend).remove();

                                var bolPossuiInputBuscaDestino = ($('input.busca', cxDestinoAppend).length) ? true : false;

                                for (var i = 0; i < arrayGrupos.length; i++) {
                                    if (bolPossuiInputBuscaDestino) {
                                        $('input.busca', cxDestinoAppend).before(montarLajotinhaGrupo(arrayGrupos[i]));
                                    } else {
                                        $(cxDestinoAppend).append(montarLajotinhaGrupo(arrayGrupos[i]));
                                    }
                                }

                                $('div.seletor_item.grupo,div.seletor_item.parcial', cxDestinoAppend).qtip(qtipOptions);

                                for (var i = 0; i < arrayUsuariosSelecionados.length; i++) {
                                    if (bolPossuiInputBuscaDestino) {
                                        $('input.busca', cxDestinoAppend).before(montarLajotinhaUsuario(arrayUsuariosSelecionados[i], false));
                                    } else {
                        ...
                        function checkarTodosUsuarios(idEscola, idUnidade, idEnsino, idSerie, idTurma, tipo, bolCoordenador, nomeGrupo, este) {
                            ...
                            ajaxTotalUsuarios = $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    este.val("Desfazer Seleção (" + este.data("qtdusuarios") + ")");
                        ...
                        function carregarUsuarios(idEscola, intInicio, intFim, idUnidade, idEnsino, idSerie, idTurma, tipo, scrollMode) {
                            ...
                            if (typeof bolCoordenador === "undefined" || typeof bolCoordenador === "false" ) {
                                bolCoordenador = false;
                            }

                            if(bolCoordenador == true && opcoes.bolAluno == true){
                            ...
                            if (tipo != tipoTurma) {
                                ajaxCarregarUsuarios = $.ajax({
                                    ...
                                    data: {
                                        ...
                                        idTurma: idTurma,
                                        intTipo: tipo,
                                        bolCoordenador: bolCoordenador // $("#bolCoordenador").val()
                                    },
                                    dataType: "json",
                                    success: function (data) {
                                        var erro = parseInt(data.error);
                                ...
                                getTurmas(idEscola, intInicio, intFim, idUnidade, idEnsino, idSerie, idTurma, tipo, scrollMode, false);
                            }
                            ...
                            $.fancybox({
                                ...
                                afterShow: function () {
                                    ...
                                    if (grupoVisualizacao.nomeGrupo === "") {
                                        grupoVisualizacao.nomeGrupo = $(".seletor_item.grupo[data-idgrupo=" + grupoVisualizacao.idGrupo + "]").attr('data-nome');
                                    }
                        ...
                        function pegarGrupoSelecionados(){
                            for (var i = 0; i < arrayGrupoAux.length; i++) {
                                if (arrayGrupoAux[i].isTurma) {
                                    arrayUsuariosSelecionados.push(arrayUsuariosSelecionados[i]);
                                    //arrayGrupo.push();
                                    console.log("grupo selecionados " + JSON.stringify(arrayUsuariosSelecionados[i]));
                                }
                            }
                        }
                        ...
                        function iniciar() {
                            ...
                            if( souAluno == "True" ){
                                ...
                                if ($(".selecionarTipoUsuarioSeletor").parent(".active").children(":first").attr("tipo") == "aluno") {
                            ...
                            if( souAluno == "True"  ){
                                ...
                                $(".selecionarTipoUsuarioSeletor").each( function(){
                            ...
                            console.log('tipoGlobalSelecionado '+tipoGlobalSelecionado);
                        ...
                        function montarLajotinhaUsuario(u, flag) {
                            var ladjota = "<div class=\"seletor_item usuario\" data-idusuario=\"" + u.idUsuario + "\">"
                                + "<img src=\"" + u.strFoto + "\" >"
                                + "<span>" + u.strNome + "</span>";
                            if ($('#aba_tarefas').hasClass('atual')) {
                                ladjota += "<a href=\"javascript:void(0);\" class=\"opcao_excluir\" alt=\"Excluir da lista\" onclick=\"excluirFiltroTarefa(0, 0, 0)\"></a>";
                            } else if ($('#aba_caminhos').hasClass('atual')) {
                                ladjota += "<a href=\"javascript:void(0);\" class=\"opcao_excluir\" alt=\"Excluir da lista\" onclick=\"excluirFiltroCaminho(0, 0, 0)\"></a>";
                            } else if ($('#aba_agendadas').hasClass('atual')) {
                                ladjota += "<a href=\"javascript:void(0);\" class=\"opcao_excluir\" alt=\"Excluir da lista\" onclick=\"excluirFiltroAgendadas(0, 0, 0)\"></a>";
                            } else {
                                ladjota += "<a href=\"javascript:void(0);\" class=\"opcao_excluir\" alt=\"Excluir da lista\"></a>";
                            }
                            ladjota += "</div>";
                        ...
                        function montarLajotinhaGrupo(g) {
                            ...
                            */
                            ...
                            //icone
                            var iconeLajotaNovaHome = 'todos';
                            switch (g.tipo) {
                                case 1: //Professores
                                    iconeLajotaNovaHome = 'professor';
                                    break;
                                case 2: //Alunos
                                    iconeLajotaNovaHome = 'aluno';
                                    break;
                                case 3: //Pais e responsaveis
                                    iconeLajotaNovaHome = 'responsavel'
                                    break;
                                case 4: //Seguidores
                                    iconeLajotaNovaHome = 'grupo seguidores';
                                    nomeGrupoAux = textoTooltip = 'Meus seguidores';
                                    break;
                                case 5: //Administrador
                                    iconeLajotaNovaHome = 'admin';
                                    break;
                            }
                            ...
                            var ladjota = '<div title="' + textoTooltip
                                + '" class="seletor_item '
                                + ((!g.bolCompleto) ? 'parcial ' : 'grupo ')
                                + iconeLajotaNovaHome
                                + '" data-idgrupo="'
                                + g.idGrupo
                                + '" data-nome="'
                                + g.nomeGrupo + '" >'
                                + '<a href="javascript:void(0);" class="ver_todos_tool" data-idgrupo="'
                                + g.idGrupo
                                + '"><span>'
                                + nomeGrupoAux
                                + '</span></a>'
                                + '<a href="javascript:void(0);" class="opcao_excluir" alt="Excluir da lista"></a></div>';
                        ...
                        function montarLajotinhaTodos(elemListaLajotinhas) {
                            if (opcoes.strLajotinhaTodos != null) {
                                if ($('.seletor_item', elemListaLajotinhas).length == 0) {
                        ...
                        function getQtdTotalUsuarios() {
                            ...
                            if (tipo != tipoTurma) {
                                ...
                                $(".selecionarTodos").data("qtdusuarios", totalDeTurmas);
                                $(".selecionarTodos").val($(".selecionarTodos").val() + " (" + totalDeTurmas + ")");
                                $(".selecionarUsersSeletor .caixa_busca").css("left", ($(".selecionarTodos").width() + 30) + "px");
                            }
                        }

                        function ListarDisciplinas() {
                            var turma = $('#turmasSeletor').val();
                            if (Materias != null && Materias.readyState < 4) {
                                Materias.abort()
                            }
                            $("#disciplinasSeletor").children('option').remove();
                            $("#disciplinasSeletor").append(new Option("Todas as Disciplina", 0));
                            if (turma > 0) {
                                Materias = $.ajax({
                                    ...
                                    success: function (aq) {
                                        if (aq.materias.length > 0) {
                                            $.each(aq.materias, function (ix, item) {
                                                $("#disciplinasSeletor").append(new Option(item.strMateria, item.IdMateria));
                                            });
                                        }
                                    },
                                    error: function (ap) {
                        ...
                        function atualizarBotaoSelecionarTodos(nomeGrupo, tipo) {
                            ...
                            console.log("atualizarBotaoSelecionarTodos");
                                ...
                                if (arrayGrupoUltimaAcao.length > 0) {
                                    for (var i = 0; i < arrayGrupoUltimaAcao.length; i++) {
                                        if (arrayGrupoUltimaAcao[i].nomeGrupo == nomeGrupo && arrayGrupoUltimaAcao[i].tipo == tipo) {
                                            $(".selecionarTodos").val("Desfazer Seleção");
                        ...
                        //listar turmas :::
                        function getTurmas(idEscola, intInicio, intFim, idUnidade, idEnsino, idSerie, idTurma, tipo, scrollMode, check) {
                            if (x != null && x.readyState < 4) {
                                x.abort()
                            }

                            //Essa parte controla o filtro do modal de criar tarefas.
                            // Comentário by: Josias 
                            ///Importante:
                            /**
                             *  idEnsino representa o select dos níveis. 
                             * No caso 
                             * idEnsino = 0 - Educação Infantil.
                             * idEnsino = 1 - Ensino Fundamental.
                             * idEnsino = 2 - Ensino Médio.
                             * 
                             *  Após o primeiro filtro temos o idSérie onde ( 0 - 1°ano  )  :
                             *  idSerie = 6 - representa 7 ° ano
                             *  idSerie = 5 - reprsenta o 6° ano
                             */
                        ...
                        html = html + "<form id=\"seletorGlobal\" name=\"formSeletor\" onsubmit=\"return false;\">"
                            + "	<input type=\"text\" class=\"busca\" placeholder=\"Digite um nome\">"
                            + "	<a href=\"javascript:void(0);\" class=\"seletor_adicionar\" title=\"Selecionar usuários\"></a>";

                        if (opcoes.caixaLajotaExterna == null) {
                            html = html + "	<div class=\"seletor_lista\">"
                                + "		<!--<div class=\"seletor_item\">"
                                + "			<a href=\"javascript:void(0);\" alt=\"\">Nome do aluno</a>"
                                + "			<a href=\"javascript:void(0);\" class=\"opcao_excluir\" alt=\"Excluir da lista\">X</a>"
                                + "		</div>-->"
                        ...
                        html2 += '<form id="#seletorGlobal" name="formSeletor" onsubmit="return false;">';

                        if (opcoes.caixaLajotaExterna == null) {
                            html2 += '<div class="seletor_lista"><input type="text" class="busca" placeholder="Digite um nome"></div>' +
                                '<a href="javascript:void(0);" class="seletor_adicionar" title="Selecionar usuários"></a>';
                        } else {
                            html2 += '<input type="text" class="busca" placeholder="Digite um nome">'
                                + '<a href="javascript:void(0);" class="seletor_adicionar" title="Selecionar usuários"></a>';
                        ...
                        if (opcoes.caixaLajotaExterna != null) {
                            ...
                            opcoes.caixaLajotaExterna.html("<div class=\"seletor_lista\">"
                                + "		<!--<div class=\"seletor_item\">"
                                + "			<a href=\"javascript:void(0);\" alt=\"\">Nome do aluno</a>"
                                + "			<a href=\"javascript:void(0);\" class=\"opcao_excluir\" alt=\"Excluir da lista\">X</a>"
                                + "		</div>-->"
                            ...
                            $('.seletor_lista', $html).bind('click', function (e) {
                                console.log('Lista');
                                if (!($(e.target).hasClass('seletor_item') || $(e.target).closest('div').hasClass('seletor_item') || $(e.target).is(':input'))) {
                                    $('.busca', this).focus();
                                }
                            });
                        }
                        ...
                        if (opcoes.caixaLajotaExterna != null) {
                            montarLajotinhaTodos($('.seletor_lista', opcoes.caixaLajotaExterna));
                            $(opcoes.caixaLajotaExterna).on("click", ".seletor_lista .ver_todos_tool", function (e) {

                                console.log('S');
                                e.preventDefault();
                                var idGrupo = $(this).closest(".seletor_item").data("idgrupo");
                                var grupo = "";
                            ...
                            montarLajotinhaTodos($('.seletor_lista', principal));
                            $('body').on("click", ".seletor_lista .ver_todos_tool", function (e) {
                                console.log('Selector Lista');
                                e.preventDefault();
                        ...
                        /** Deletar Lajotinhas **/
                        if (opcoes.caixaLajotaExterna != null) {
                            $(opcoes.caixaLajotaExterna).on("click", ".seletor_lista .seletor_item .opcao_excluir", function (e) {
                                if ($(this).closest(".seletor_item").hasClass("grupo")) {
                                    var idGrupo = parseInt($(this).closest(".seletor_item").data("idgrupo"));
                                    ...
                                    $(this).closest(".seletor_item").remove();
                                    montarLajotinhaTodos($('.seletor_lista', opcoes.caixaLajotaExterna));
                                } else {
                                    var idUsuario = parseInt($(this).closest(".seletor_item").data("idusuario"));
                                ...
                                if (opcoes.caixaLajotaExterna.find(".seletor_lista").children().size() > 0) {
                                    if (opcoes.botaoConclusao == null) {
                                        liberarBotaoInserirExterno(true);
                                    }
                                } else {
                            ...
                            $(this).on("click", ".seletor_lista .seletor_item .opcao_excluir", function (e) {
                                console.log('.seletor_lista .seletor_item .opcao_excluir');
                                if ($(this).closest(".seletor_item").hasClass("grupo")) {
                                    var idGrupo = parseInt($(this).closest(".seletor_item").data("idgrupo"));
                                ...
                                if (principal.find(".seletor_lista").children().size() > 0) {
                                    if (opcoes.botaoConclusao == null) {
                                        liberarBotaoInserirExterno(true);
                                    }
                                } else {
                        ...
                        $(this).find(".busca").autocomplete({
                            source: function (request, response) {
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        principal.find(".busca").removeClass("ui-autocomplete-loading");
                                    },
                                    error: function (data) {
                                        console.log(data.responseText);
                                    }
                            ...
                            select: function (event, ui) {
                                ...
                                if (opcoes.caixaLajotaExterna != null) {
                                    if ($('.seletor_lista input.busca', opcoes.caixaLajotaExterna).length) {
                                        $('.seletor_lista input.busca', opcoes.caixaLajotaExterna).val("").before(montarLajotinhaUsuario(usuario, opcoes.bolLajota));
                                    } else {
                                        $('.seletor_lista', opcoes.caixaLajotaExterna).append(montarLajotinhaUsuario(usuario, opcoes.bolLajota));
                                    }
                                    $('.seletor_item.todos', opcoes.caixaLajotaExterna).remove();
                                } else {
                                    $('.seletor_lista input.busca', principal).val("").before(montarLajotinhaUsuario(usuario, opcoes.bolLajota));
                                    $('.seletor_item.todos', principal).remove();
                                    liberarBotaoInserirExterno(true);
                                }
                        ...
                        //Cria o Modal
                        $(this).find(".seletor_adicionar").click(function (e) {
                            console.log('entrou jquery.AvaSelectorNovaHome seletor_adicionar :::');
                            e.preventDefault();

                            //arrayUsuarioUltimaAcao = [];
                            //arrayGrupoUltimaAcao = [];
                            //
                            ...
                            $.fancybox({
                                ...
                                afterShow: function () {
                                    ...
                                    /** Botão Selecionar Todos (Carteirinha) **/
                                    $(".selecionarTodos").click(function (e) {
                                        ...
                                        if (parseInt(este.data("selecionado")) == 1) {
                                            ...
                                            console.log('Aqui no 1');
                                            
                                            if (tipo == tipoTurma) {
                                                console.log("opcao turmas1");
                                            }
                                    ...
                                    $("#listaCarteirinhaSeletorSelecionado").on("click", ".acoesFeed .remover", function (e) {
                                        console.log('listaCarteirinhaSeletorSelecionado');
                                        e.preventDefault();
                                    ...
                                    $(".selecionarUsersSeletor").on("click", ".carteirinha_seletor", function (e) {
                                        ...
                                        console.log(e);
                                        console.log("selecionarUsersSeletor");
                                        e.preventDefault();

                                        if ($(this).hasClass("selecionarTudoGrupo")) { //esta em grupo

                                            console.log('selecionarTudoGrupo');
                                            

                                            if (opcoes.bolLajota) {

                                            console.log('opcoes.bolLajota');

                                                if (arrayGrupoUltimaAcao.length > 0) {

                                                    console.log('arrayGrupoUltimaAcao.length');
                                                ...
                                                if (arrayGrupos.length > 0) {
                                                    ...
                                                    console.log('arrayGrupos.length');
                                                ...
                                                console.log('ota else');
                                                

                                                if (arrayGrupos.length > 0) {

                                                    console.log('else arrayGrupos');
                                                    for (var i = 0; i < arrayGrupos.length; i++) {
                                                        for (var o = 0; o < arrayGrupos[i].usuarios.length; o++) {
                                                            if (arrayGrupos[i].usuarios[o].idUsuario == parseInt($(this).data("idusuario"))) {
                                                                arrayGrupos[i].usuarios.splice(o, 1);
                                            ...
                                            // verificar se é uma turma
                                            //...
                                            console.log('não está em grupo');

                                            var idUser = parseInt($(this).data("idusuario"));
                                            var isTurma = false;
                                                ...
                                                if ($(this).hasClass("carteirinha_selected")) {
                                                    ...
                                                    if (!flagGrupo) {
                                                        ...
                                                        if (opcoes.bolLajota) {
                                                            for (var i = 0; i < arrayUsuarioUltimaAcao.length; i++) {
                                                                // verifica se é truma
                                                                if (!usuario.isTurma && arrayUsuarioUltimaAcao[i].idUsuario == usuario.idUsuario) {
                                                                    flag = true;
                                                                }
                                                            }
                                                        } else {
                                                        ...
                                                        if (!flag) {
                                                            if (opcoes.bolLajota) {
                                                                ...
                                                                /*}
                                                                    ...
                                                                    $.each(usuario.alunos, function(index, value){
                                                                        //arrayGrupoUltimaAcao.push( //alterei para a linha de baixo
                                                                        arrayUsuarioUltimaAcao.push(
                                                                            {
                                    ...
                                    $(".selecionarTipoUsuarioSeletor").click(function (e) {
                                        ...
                                        var intervalTrocaAbaSeletor = window.setInterval(function () {
                                            if (!ajaxcarregarComboSeletor) {
                                                ...
                                                var nomeGrupo = getNomeGrupo(tipoGlobalSelecionado);
                                                //:::
                                                console.log("nomeGrupo " + nomeGrupo + " tipoGlobalSelecionado " + tipoGlobalSelecionado);
                                                atualizarBotaoSelecionarTodos(nomeGrupo, tipoGlobalSelecionado);
                                ...
                                beforeClose: function () {
                                    ...
                                    if (opcoes.bolLajota && fechouCancelar && opcoes.bolAtualizarLajotaQuandoCancelar) {
                                        ...
                                        $('.seletor_item:not(.todos)', cxDestinoAppend).remove();
                                        ...
                                        $('div.seletor_item.grupo', cxDestinoAppend).qtip(qtipOptions);
                                        ...
                                        for (var i = 0; i < arrayUsuariosSelecionados.length; i++) {
                                            ...
                                            $('.seletor_item.todos', cxDestinoAppend).remove();
                                        }
                                        if (cxDestinoAppend.children().size() > 0) {
                                            if (opcoes.botaoConclusao == null) {
                        ...
                        if (opcoes.botaoConclusao == null) {
                            ...
                            $(this).find(".submeter").click(function (e) {
                                ...
                                if (principal.find(".seletor_item").size() > 0) {
                                    if (opcoes.bolLajota) {
                                        ...
                                        arrayGrupos.splice(0, arrayGrupos.length);
                                        if (opcoes.caixaLajotaExterna != null) {
                                            $('.seletor_item', opcoes.caixaLajotaExterna).remove();
                                        } else {
                                            $('.seletor_item', principal).remove();
                                        }
                                    } else {
                                    ...
                                    if (cxDestino.find(".seletor_lista").children().size() > 0) {
                                        if (opcoes.caixaLajotaExterna == null) {
                                            liberarBotaoInserirExterno(true);
                                        }
                                    } else {
