...
<% 
    var assuntosParaEdicao = new List<Grupo.Models.Assunto>();
...
<div id="criareditar" class="cont_criar fancyAssuntos" style="display:block;">
    ...
    <div class="_feed_lista_assunto">
        ...
        <ul>
            <li class="pdnl20 liassufixo"><%=assuntoGeral.strAssunto%></li>
            <li class="pdnl20 liassufixo">
            ...
            <% foreach(var a in assuntosParaEdicao) { 
               bool bolAssuntoComPost = assuntosComPost.Contains(a.id);
            %>                       
            <li assu="<%=a.id%>" class="liassunto">
    ...
    <div class="feed_confirma">        
        <h2>Remover o assunto "Nome do assunto"?</h2>
        <strong>
        ...
        <input type="hidden" id="hAssuntoEditar" initvalue="<%=assuntoGeral.id %>" value="<%=assuntoGeral.id %>" />
        <div class="bootstrap">
			<div class="btn-group">                     
                ...
                <ul class="dropdown-menu" id="cbAssuntoEditar">
                    <% foreach (var assu in Model)
                        { %>    	
                        <li assu="<%=assu.id%>">
                            <input type="checkbox" id="ckAssuntoEditar<%=assu.id%>" <%=(assu.id == assuntoGeral.id) ? "checked=\"checked\"" : "" %> />
			                <label for="ckAssuntoEditar<%=assu.id%>"><%=assu.strAssunto%>&nbsp;</label>
			            </li>                        
                    <% } %>	                                                     
			    </ul>
			</div>  
